...
func (handler *Handler) Withdraw(amount int) (err error, withdraw_amount int, balance int, message *string) {
	...
	if inventory <= 0 {
		return errors.New("Unable to process your withdrawal at this time."), 0, 0, nil
	}

	account := handler.accountMap.GetAccount(accountID)

	if account.GetBalance() < 0 {
		return errors.New("Your account is overdrawn! You may not make withdrawals at this time."), 0, 0, nil
	}
